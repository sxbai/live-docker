...
def proxy_download_file():
    ...
    if downloader_switch:
        ...
        def get_url_and_headers():
            return AliyunDrive()._get_download_url(params, token), headers.copy()
        ...
        download_url = AliyunDrive()._get_download_url(params, token)
...
def proxy():
    ...
    try:
        r = requests.get(url=url, headers=header, stream=True, proxies=proxies)
