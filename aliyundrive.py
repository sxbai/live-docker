...
class AliyunDrive():
    ...
    def _get_download_url(self, params, token):
        ...
        file_id = json.loads(r.text)['responses'][0]['body']['file_id']
    ...
    def _get_cache(self, key):
        if key in self.cache:
            data = self.cache[key]
            if data:
                if 'expires_at' not in data or data['expires_at'] >= int(
                        time.time()):
                    return data
    ...
    #spider = AliyunDrive()
    #res = spider.list_items(parent_item={'type': 'directory', 'id': 'https://www.aliyundrive.com/s/bK2yk6nHWhi', 'name': '阿里盘搜：显微镜下的大明之丝绢案-首播4集', 'cover': '', 'description': '更新时间：2023-02-14', 'cast': [], 'director': '', 'area': '', 'year': 0, 'sources': [], 'danmakus': [], 'subtitles': [], 'params': {'type': 'video', 'pf': 'ps', 'num': 3}})
    #res = spider._get_download_url({'template_id': '', 'share_id': 'bK2yk6nHWhi', 'file_id': '6401fa41aa573649a2bc41a2b9f45631ddd2cb6d', 'drive_id': '1734471120', 'pf': 'ali'}, '')
